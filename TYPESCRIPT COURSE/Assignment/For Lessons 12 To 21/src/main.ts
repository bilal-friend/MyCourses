...
function yesOrNo(val: number | string) {
  return typeof val === "string" ? new Error("Error") : val < 10;
}
...
function isHeOld(age: number): custom | string {
  if (typeof age === "string") return new Error("Error");
